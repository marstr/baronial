...
var commitCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		state, err := index.LoadState(ctx, targetDir)
		if err != nil {
			logrus.Fatal(err)
		}

		budgetBal := state.Budget.RecursiveBalance()
...
func init() {
	...
	calculatedAmountFromChanges, err := calculateAmount(ctx, ".")
	if err != nil {
		logrus.Warnf("Falling back to naive default amount %q because of the following error: %s", amountDefault, err)
	}
	...
	commitCmd.Flags().StringP(amountFlag, amountShorthand, calculatedAmountFromChanges, amountUsage)
...
func calculateAmount(ctx context.Context, targetDir string) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 2*time.Second)
	defer cancel()

	targetDir, err := index.RootDirectory(targetDir)
	if err != nil {
		return amountDefault, err
	}
	...
	return envelopes.CalculateAmount(*head.State, *updated).String(), nil
}
